...
class NotificationsController extends Controller
{
    ...
    /**
     * Mark all notifications as seen (mimics old API: get_notifications.php?seen=1)
     * 
     * @param Request $request
     * @return JsonResponse
     */
    public function markAllSeen(Request $request): JsonResponse
    {
        ...
        // Get all unseen notification IDs for this user (same logic as old WoWonder)
        $notificationTypes = [
            'requested_to_join_group', 
            'interested_event', 
            'going_event', 
        ...
        $unseenNotifications = DB::table('Wo_Notifications')
            ->where('recipient_id', $tokenUserId)
            ->where('seen', 0)
            ->whereIn('type', $notificationTypes)
            ->pluck('id')
        ...
        if (!empty($unseenNotifications)) {
            // Update seen to current timestamp (same as old WoWonder: SET seen = time())
            $updatedCount = DB::table('Wo_Notifications')
                ->whereIn('id', $unseenNotifications)
                ->update(['seen' => time()]);
        }
